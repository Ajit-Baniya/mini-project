...
if (isset($_POST['submit'])) {
    ...
    $from = new DateTime(date("Y-m-d", strtotime($fromdate)));
    $datediff = $todayDate->diff($from)->format('%r%a');
    if ($datediff < 0) {
        echo "<script>alert('Please choose upcoming dates!');</script>";
    ...
    $query0->execute();
    $result0 = $query0->fetch(PDO::FETCH_OBJ);
    if ($result0->Quantity < 1) {
        echo "<script>alert('This car is not available currently. Please try later');</script>";
        echo "<script type='text/javascript'> document.location = 'car-listing.php'; </script>";
        return;
    }
              ...
              <div class="main_features">
                <ul>
                  ...
                  <li> <i class="fa fa-archive" aria-hidden="true"></i>
                    <h5><?php echo htmlentities($available = $result->Quantity); ?></h5>
                    <p>Available</p>
                  </li>
                </ul>
              </div>
